...
def _ctx(now, weather=None, health=()):
    return Context(now=now, mono_ms=0, profile=Profile.DAY, weather=weather, health=health)


def test_weather_source_none_without_data():
    s = WeatherRundownSource(make_scene=lambda w, now: "WX")
